...
LazySnapping::LazySnapping(const cv::Mat& maskImage, const std::vector<cv::Vec3b>& nodeColors, const std::vector<Connection>& connections)
	: m_maskImage(maskImage), m_nodeColors(nodeColors), m_connections(connections)
{
...
bool LazySnapping::Process(cv::Mat& paintImage, int clusterNum /* = 64 */, bool showSegmentation /* = false */)
{
	if (!setMarkPoints(paintImage, clusterNum))
		return false;
	runMaxFlow();
	BuildSegmentation();
	if (showSegmentation)
...
bool LazySnapping::setMarkPoints(cv::Mat& paintImage, int clusterNum)
{
	...
	int foreClusterNum = min(static_cast<int>(m_foreComps.size()), clusterNum);
	int backClusterNum = min(static_cast<int>(m_backComps.size()), clusterNum);
	// Todo: adjust kmeans parameters.
	kmeans(foreColors, foreClusterNum, foreLabels, TermCriteria(CV_TERMCRIT_EPS + CV_TERMCRIT_ITER, 15, 0.5), 4, KMEANS_PP_CENTERS);
	kmeans(backColors, backClusterNum, backLabels, TermCriteria(CV_TERMCRIT_EPS + CV_TERMCRIT_ITER, 15, 0.5), 4, KMEANS_PP_CENTERS);
	...
	for (int i = 0; i < foreLabels.size(); i++)
	{
...
{
	// Add nodes.
	for (int i = 0; i < m_nodeColors.size(); i++)
	{
		m_graph->add_node();
...
{
	if (compId < 1 || compId > m_nodeColors.size())
...
float LazySnapping::calE2(int compA, int compB)
{
	...
	float epsilon = 0.01;	// 0.01
	float lamada = 100;	// 100

	return lamada / (epsilon + distance);
}
