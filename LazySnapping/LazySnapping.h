...
{
public:
	LazySnapping(const cv::Mat& maskImage, const std::vector<cv::Vec3b>& nodeColors, const std::vector<Connection>& connections);
	~LazySnapping();
...
public:
	...
	/// <param name="clusterNum">The cluster number for kmeans algorithm.</param>
	...
	bool Process(cv::Mat& paintImage, int clusterNum = 64, bool showSegmentation = false);
...
private:
	...
	/// <param name="clusterNum">The cluster number for kmeans algorithm.</param>
	/// <returns>True for successful operation.</returns>
	bool setMarkPoints(cv::Mat& paintImage, int clusterNum);
